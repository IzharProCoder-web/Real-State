...
const FeaturedComponent = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 1024) {
        setSlidesToShow(3); // Large screens: show 3 slides
      } else if (window.innerWidth >= 768) {
        setSlidesToShow(2); // Medium screens: show 2 slides
      } else {
        setSlidesToShow(1); // Small screens: show 1 slide
      }
    };
  ...
  const handleNext = () => {
    setCurrentSlide((prev) => (prev + 1) % (featuredData.length - slidesToShow + 1));
  };

  const handlePrev = () => {
    setCurrentSlide((prev) =>
      prev === 0 ? featuredData.length - slidesToShow : prev - 1
    );
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center border-t-2 border-[#262626] mt-8 pt-4">
          <div className="w-full sm:w-auto mb-4 sm:mb-0">
            <p className="text-white text-center sm:text-left">
              Slide {currentSlide + 1} of {featuredData.length - slidesToShow + 1}
            </p>
          </div>
